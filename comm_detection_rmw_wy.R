# PARTITIONING ALGORITHM (WYOMING)
...
dates_wy <- unique(df_wy_d$measure_date)
dfs_list_wy <- lapply(dates_wy, function(d) summarize_fn(df_wy_d %>% filter(measure_date==d) %>% select(-measure_date)))
...
gs <- lapply(dates_wy, function(x) graph.data.frame(df_wy_d %>% filter(measure_date == x) %>% select(-measure_date)))

# overall
dfa2n <- cbind(dfa2_wy %>% select(c('origin_county', 'dest_county')), dfa2_wy %>% select(-c('origin_county', 'dest_county')))
...
for(i in 1:length(adjs)){
  ...
  labels_colors(dend_wy) <- ordered_labels$color
  for(k in 2:6){
    png(paste0("./CSTE/Figures/cluster_wy_", k, "groups_color_by_region_", labs2[i], ".png"), height=15, width=9, units='in', res=300)
    ...
    ggsave(paste0("./CSTE/Figures/new_regions_wy_", k, "groups_", labs2[i], ".png"), height=7, width=7, units="in", dpi=300)
  }
}

png("./CSTE/Figures/dend_wy.png", width=400, height=250, units="mm", res=600)
plot_dendrogram(fgc)
