...
dates_id <- unique(df_id_d$measure_date)
dfs_list_id <- lapply(dates_id, function(d) summarize_fn(df_id_d %>% filter(measure_date==d) %>% select(-measure_date)))
...
gs <- lapply(dates_id, function(x) graph.data.frame(df_id_d %>% filter(measure_date == x) %>% select(-measure_date)))

# overall
dfa2n <- cbind(dfa2_id %>% select(c('origin_county', 'dest_county')), dfa2_id %>% select(-c('origin_county', 'dest_county')))
...
for(i in 1:length(adjs)){
  ...
  labels_colors(dend_id) <- ordered_labels$color
  for(k in 2:6){
    png(paste0("./CSTE/Figures/cluster_id_", k, "groups_color_by_region_", labs2[i], ".png"), height=15, width=9, units='in', res=300)
    ...
    ggsave(paste0("./CSTE/Figures/new_regions_id_", k, "groups_", labs2[i], ".png"), height=7, width=7, units="in", dpi=300)
  }
}

png("./CSTE/Figures/dend_id.png", width=400, height=250, units="mm", res=600)
plot_dendrogram(fgc)
title(adj=0.5, "Idaho")
dev.off()
